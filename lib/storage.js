...
function setAppStatus(guildId, type, isOpen) {
  const all = loadAll();
  if (!all[guildId]) all[guildId] = {};
  if (!all[guildId].applications) {
    all[guildId].applications = { media: true, staff: false, helper: false };
  }

  if (type === 'all') {
    all[guildId].applications.media = isOpen;
    all[guildId].applications.staff = isOpen;
    all[guildId].applications.helper = isOpen;
  } else {
    all[guildId].applications[type] = isOpen;
  }

  saveAll(all);
  return all[guildId].applications;
}
