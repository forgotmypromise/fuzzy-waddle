...
// Commands that can be granted via /whitelist setperms
...
const commandDefs = [
    ...
    new SlashCommandBuilder()
        .setName('free')
        .setDescription('Send a free-script style message.')
    ...
    new SlashCommandBuilder()
        .setName('whitelist')
        .setDescription('Manage the bot whitelist and per-command permissions.')
        .addSubcommand(sub =>
            sub
                .setName('add')
                .setDescription('Add a user to the whitelist (all restricted commands by default).')
        ...
        .addSubcommand(sub =>
            sub
                .setName('list')
                .setDescription('Show the whitelist and each user\'s allowed commands.')
        )
        .addSubcommand(sub =>
            sub
                .setName('setperms')
                .setDescription('Set which commands a whitelisted user can use.')
                .addUserOption(option =>
                    option
                        .setName('user')
                ...
                .addStringOption(option =>
                    option
                        .setName('commands')
                        .setDescription('Comma-separated commands, or "all". Example: genkeys,disable,freescript')
                        .setRequired(true)
                )
        ),
    ...
    new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option
                .setName('duration')
                .setDescription('How long the key lasts')
                .setRequired(false)
                .addChoices(
                    { name: '1 Day', value: '1d' },
        ...
        .addStringOption(option =>
            option
                .setName('note')
                .setDescription('Optional note shown on the key (e.g. -giveaway, -vip)')
                .setRequired(false)
        ),
    ...
    new SlashCommandBuilder()
        .setName('setfreescript')
        .setDescription('Set the saved free-script message (whitelist only).')
        .addStringOption(option =>
            option
                .setName('text')
                .setDescription('Message the bot will post with /freescript')
                .setRequired(true)
        ),

    new SlashCommandBuilder()
        .setName('freescript')
        .setDescription('Post the free-script message for everyone to see (whitelist only).')
        .addStringOption(option =>
            option
                .setName('text')
                .setDescription('Optional custom text (uses saved message if omitted)')
                .setRequired(false)
        ),
    ...
    // Allow use in DMs (and servers)
    if (typeof command.setDMPermission === 'function') {
        command.setDMPermission(true);
    }
