...
const commandDefs = [
  ...
  new SlashCommandBuilder()
    .setName('apply')
    .setDescription('Apply for a staff/media role')
    .addStringOption((opt) =>
      opt
        .setName('role')
  ...
  new SlashCommandBuilder()
    .setName('setstatus')
    .setDescription('Set your support status (shown when tickets open)')
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
    .addStringOption((opt) =>
      opt
        .setName('time')
        .setDescription('Until when? Examples: 12am, 00:00, 3:30pm, 15:30')
        .setRequired(true)
    )
    .addStringOption((opt) =>
      opt
        .setName('reason')
        .setDescription('Why you are unavailable')
        .setRequired(true)
        .addChoices(
          { name: 'Asleep / Sleeping', value: 'asleep' },
          { name: 'On Break', value: 'break' },
          { name: 'Busy / Away', value: 'busy' },
          { name: 'Offline / Unavailable', value: 'offline' },
          { name: 'Working on something else', value: 'working' }
        )
    ),
].map((cmd) => cmd.toJSON());
